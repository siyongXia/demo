<template>
    <div>
        <div class="docinfo layout" @click="loadDocHomepage(doclist.unit_id,doclist.dep_id,doclist.doctor_id)">
            <img class="avatar inblo" :src="doclist.icon" :onerror="nothumb">
            <div class="rgtbox fs14 typo_gray">
                ...
                <div class="mt2 elli deplist lh22">
                    <i>{{doclist.prev_work_unit?'曾就职于'+doclist.prev_work_unit:'第一执业'+doclist.first_practice}}</i> 
                ...
                <div class="lh18 fs14">
                    评分 <i class="typo_lorange">{{doclist.comment_total_score}}</i>&nbsp;&nbsp;
                    预约量 <i class="typo_lorange">{{doclist.history_register_total >9999 ? Math.trunc(doclist.history_register_total/ 1000) / 10 + '万' : doclist.history_register_total}}</i>&nbsp;&nbsp;
                    咨询量 <i class="typo_lorange">{{doclist.history_ask_total}}</i>
                </div>
                <div class="fs12 openservices-i mt4">
                    <i v-if="doclist.is_register==1">挂号</i>
                    <i v-if="doclist.is_ask==1">图文</i>
                    <i v-if="doclist.is_ask_tel==1">电话</i>
                    <i v-if="doclist.is_ask_video==1">视频</i>
                    <i v-if="doclist.is_sch==1">加号</i>
                    <i v-if="doclist.is_vip==1">私人</i>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        loadDocHomepage: function (unit_id,dep_id,doctor_id) {
            ...
            this.gaStaticClick('名医极速挂号-医生ID-'+doctor_id);
